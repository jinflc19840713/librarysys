...
exports.GetLearningRoute = async(req, res) => {
    console.log("UserStudy Controller/ GetLearning Route...")
    try{
        let studyResults = await db.sequelize.query(
            "CALL `testdb`.`Proc_GetLearningRoute`(:i_UserId);",
            {replacements:{i_UserId: req.userId}}
        )
        res.status(200).send({code:0, data: studyResults})
    }catch(err){
        console.log(err)
